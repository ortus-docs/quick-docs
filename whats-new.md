...
Add a GUID KeyType for use with `guid`qb column types.

Account for compound keys in `hasManyDeep`/`hasManyThrough`relationships.
...
* When [eager loading](guide/relationships/eager-loading.md) a relationship, if there is a [default entity](guide/relationships/relationship-types/belongsto.md#withdefault) defined, it will return the default entity instead of `null`.
* The [`hydrate`](guide/getting-started/creating-new-entities.md#hydrate) function no longer accepts a `ignoreNonExistentAttributes` flag. This is because `assignAttributesData` always skips non-existent attributes.

### Features

* Get a new filled instance of a relationship by calling the `fill` method on the relationship object.
* Add `newAttributes` and `originalAttributes` to the [`preUpdate`](guide/interception-points.md#quickpreupdate) event.
* Add [`firstOrNew`, `firstOrCreate`, `firstWhere`, `findOrNew`, and `findOrCreate`](guide/relationships/retrieving-relationships.md) methods to Relationship classes.
* Allow specifying [virtual attributes](guide/getting-started/defining-an-entity/#virtual) with a [cast](guide/getting-started/defining-an-entity/#casts).
* Allow returning a new non-persisted entity from a relationship using the [`newEntity`](guide/relationships/retrieving-relationships.md) function.
...
Respect qb's [SQL Commenter](https://qb.ortusbooks.com/query-builder/debugging/sqlcommenter) support options.
